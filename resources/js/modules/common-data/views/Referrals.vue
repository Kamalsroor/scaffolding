<template>
  <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">{{
        !$h.checkBoolean(serverParams.deleted) ? $t('g.list', {model: $t('referrals.plural')}) : $t('g.deleted', {model: $t('referrals.plural')})
      }}</h2>
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
      <a class="btn btn-primary shadow-md mr-2" v-can="['create-referral']" @click="OpenNewReferralModal()">
        <PlusIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.create') }}
      </a>
      <a v-can="['restore-referral','force-delete-referral']" v-if="$h.checkBoolean(serverParams.deleted)"
         class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,false)">
        <ListIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.list', {model: $t('referrals.plural')}) }}
      </a>
      <a v-can="['restore-referral','force-delete-referral']" v-if="!$h.checkBoolean(serverParams.deleted)"
         class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,true)">
        <Trash2Icon class="mr-2 w-5 h-5"/>
        {{ $t('g.deleted', {model: $t('referrals.plural')}) }}
      </a>
      <!-- BEGIN: Modal Content -->
      <Modal v-can="['create-referral','edit-referral']" :show="ModelIsOpen" size="modal-xl" @hidden="closeModel()">
        <form class="validate-form" @submit.prevent="save">
          <ModalHeader>
            <h2 class="font-medium text-base mr-auto">
              {{
                editMode ? $t('g.edit_row', {model: $t('referrals.plural')}) : $t('g.add_new', {model: $t('referrals.plural')})
              }}
            </h2>
          </ModalHeader>
          <ModalBody class="grid grid-cols-12 gap-8 gap-y-4">
            <InputField v-model="referral.name" :errors="v$.referral.name.$errors" class="col-span-12 sm:col-span-4"
                        :label="$t('forms.attributes.name')" name="name" :placeholder="$t('forms.attributes.name')"/>
            <Switch v-model="referral.active" :errors="v$.referral.active.$errors" class="col-span-12 sm:col-span-4"
                    des="Activate to show in frontend website" :label="$t('forms.attributes.active')"
                    name="active-input"/>
            <InputField v-model="referral.order_id" type="number" :errors="v$.referral.order_id.$errors"
                        class="col-span-12 sm:col-span-4"
                        :label="$t('forms.attributes.order_id')" name="order_id"
      ...
      >
        ...
        <template #table-row="props">
          <span v-if="props.column.field == 'actions'">
              ...
              <button v-if="!props.row.deleted" class="flex items-center btn btn-sm btn-secondary  mr-1 mb-2"
                      v-can="['edit-referral']" @click="OpenNewReferralModal(props.row.id)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2"/>
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" name="CheckSquare"/>{{ $t('g.edit') }}
              </button>

              <button v-if="props.row.deleted" class="flex items-center btn btn-sm btn-success text-white  mr-1 mb-2"
                      v-can="['restore-referral']" @click="restoreReferral(props.row.id)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2"/>
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" color="white" name="ArrowLeft"/>{{ $t('g.restore') }}
              </button>
              <button class="flex items-center btn btn-sm btn-danger mr-1 mb-2" v-can="['delete-referral']"
          ...
          <span v-if="props.column.field == 'active'">
                <Switch v-model="props.row.active" :disabled="$h.checkBoolean(serverParams.deleted)" name="active"
                        @change="statusChange(props.row.id)"/>
            </span>
...
import {Deleted, Loading, Notify} from '@/mixins'
import ReferralsController from '@/modules/common-data/controllers/ReferralsController.js';
...
export default {
  ...
  methods: {
    ...
    async getData(page = null) {
      ...
      let data = await ReferralsController.getData(this.serverParams);
      if (data) {
        this.rows = data.data;
    ...
    async getModel(id) {
      this.StartLoading();
      let data = await ReferralsController.getModel(id);
      if (data) {
        this.referral = data;
      }
      this.StopLoading();
