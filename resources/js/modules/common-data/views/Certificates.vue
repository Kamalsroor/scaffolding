<template>
  <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">{{ !$h.checkBoolean(serverParams.deleted) ?   $t('g.list', {model: $t('certificates.plural')})  : $t('g.deleted', {model: $t('certificates.plural')}) }}</h2>
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
      <a class="btn btn-primary shadow-md mr-2" v-can="['create-certificate']" @click="OpenNewCertificateModal()">
        <PlusIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.create') }}
      </a>
      <a v-can="['restore-certificate','force-delete-certificate']" v-if="$h.checkBoolean(serverParams.deleted)" class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,false)">
        <ListIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.list', {model: $t('certificates.plural')}) }}
      </a>
      <a v-can="['restore-certificate','force-delete-certificate']" v-if="!$h.checkBoolean(serverParams.deleted)" class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,true)">
        <Trash2Icon class="mr-2 w-5 h-5"/>
        {{ $t('g.deleted', {model: $t('certificates.plural')}) }}
      </a>
      <!-- BEGIN: Modal Content -->
      <Modal v-can="['create-certificate','edit-certificate']" :show="ModelIsOpen" size="modal-xl" @hidden="closeModel()">
        <form class="validate-form" @submit.prevent="save">
          <ModalHeader>
            <h2 class="font-medium text-base mr-auto">
              {{
                editMode ? $t('g.edit_row', {model: $t('certificates.plural')}) : $t('g.add_new', {model: $t('certificates.plural')})
              }}
            </h2>
          </ModalHeader>
          <ModalBody class="grid grid-cols-12 gap-8 gap-y-4">
          <InputField v-model="certificate.name" :errors="v$.certificate.name.$errors" class="col-span-12 sm:col-span-4"
                        :label="$t('forms.attributes.name')" name="name" :placeholder="$t('forms.attributes.name')"/>

            <Switch v-model="certificate.active" :errors="v$.certificate.active.$errors" class="col-span-12 sm:col-span-4"
            des="Activate to show in frontend website" :label="$t('forms.attributes.active')" name="active-input"/>
            <InputField v-model="certificate.order_id" type="number" :errors="v$.certificate.order_id.$errors" class="col-span-12 sm:col-span-4"
      ...
      >
        ...
        <template #table-row="props">
          <span v-if="props.column.field == 'actions'">
              ...
              <button v-if="!props.row.deleted" class="flex items-center btn btn-secondary  w-24 mr-1 mb-2" v-can="['edit-certificate']"  @click="OpenNewCertificateModal(props.row.id)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" name="CheckSquare"/>{{ $t('g.edit') }}
              </button>

              <button v-if="props.row.deleted" class="flex items-center btn btn-success text-white  w-24 mr-1 mb-2" v-can="['restore-certificate']" @click="restoreCertificate(props.row.id)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" color="white" name="ArrowLeft"/>{{ $t('g.restore') }}
              </button>
              <button class="flex items-center btn btn-danger w-24 mr-1 mb-2"  v-can="['delete-certificate']" @click="deleteCertificate(props.row.id)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
...
import CertificatesController from '@/modules/common-data/controllers/CertificatesController.js';
...
export default {
  ...
  methods: {
    ...
    async getData(page = null) {
      ...
      let data = await CertificatesController.getData(this.serverParams);
      if(data){
          this.rows = data.data;
    ...
    async getModel(id) {
      this.StartLoading();
      let data = await CertificatesController.getModel(id);
      if(data){
    ...
    OpenNewCertificateModal(id = null) {
      this.resetForm();
      if (id == null) {
        this.editMode = false;
      } else {
    ...
    async deleteCertificate(id , name = "") {
      this.StartLoading();
    ...
    async restoreCertificate(id , name = "") {
      this.StartLoading();
