<template>
  ...
  <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">{{ !$h.checkBoolean(serverParams.deleted) ?   $t('g.list', {model: $t('attributes.plural')})  : $t('g.deleted', {model: $t('attributes.plural')}) }}  {{ hasChildren ? parent?.name?.de : '' }}</h2>
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
      <button class="btn btn-primary shadow-md mr-2" v-can="['create-attribute']" @click="OpenNewAttributeModal()">
        <PlusIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.create') }}
      </button>
      <router-link :to="{name:'attributes'}" class="btn btn-primary shadow-md mr-2" >
        <PlusIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.back_to_parents') }}
      </router-link>
      <a v-can="['restore-attribute','force-delete-attribute']" v-if="$h.checkBoolean(serverParams.deleted)" class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,false)">
        <ListIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.list', {model: $t('attributes.plural')}) }}
      </a>
      <a v-can="['restore-attribute','force-delete-attribute']" v-if="!$h.checkBoolean(serverParams.deleted)" class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,true)">
        <Trash2Icon class="mr-2 w-5 h-5"/>
        {{ $t('g.deleted', {model: $t('attributes.plural')}) }}
      </a>

      <!-- BEGIN: View Modal Content -->
      <Modal v-can="['view-attribute']" :show="ModelViewIsOpen" size="modal-xl" @hidden="closeViewModel()">
        <form class="validate-form" @submit.prevent="save">
          <ModalHeader>
            <h2 class="font-medium text-base mr-auto">
              {{
               $t('g.view_row', {model: $t('attributes.plural')})
              }}
            </h2>
          </ModalHeader>
          <ModalBody class="">
                ...
                <dl class="grid grid-cols-4 gap-4">
                  ...
                  <div v-for="(local, index) in locales"  class="col-span-4  grid grid-cols-3 gap-3 border-b border-gray-200" :key="index">
                    ...
                    <div class="sm:col-span-1">
                      <dt class="text-sm font-medium text-gray-500">name {{local}}</dt>
                      <dd class="mt-1 text-sm text-gray-900">{{attribute.name[local]}}</dd>
                    </div>
      ...
      <!-- BEGIN: Modal Content -->
      <Modal v-can="['create-attribute','edit-attribute']" :show="ModelIsOpen" size="modal-xl" @hidden="closeModel()">
        <form class="validate-form" @submit.prevent="save">
          <ModalHeader>
            <h2 class="font-medium text-base mr-auto">
              {{
                editMode ? $t('g.edit_row', {model: $t('attributes.plural')}) : $t('g.add_new', {model: $t('attributes.plural')})
              }}
            </h2>
          </ModalHeader>
            ...
            <TabGroup class="col-span-12 sm:col-span-12" @change="changeTab">
              ...
              <TabPanels class="border-l border-r border-b ">
                <template v-for="(local, index) in locales" :key="index">
                  ...
                  <TabPanel class="p-5">
                    <InputField v-model="attribute.name[local]" :errors="v$.attribute.name[local].$errors" class="col-span-12 sm:col-span-4"
              ...
              <InputField type="number" v-model="attribute.order_id" :errors="v$.attribute.order_id.$errors" class="col-span-12 sm:col-span-4"
                          :label="$t('forms.attributes.order_id')" name="order_id" :placeholder="$t('forms.attributes.order_id')"/>

            <Switch v-model="attribute.active" :errors="v$.attribute.active.$errors" class="col-span-12 sm:col-span-4" :label="$t('forms.attributes.active')" name="active-input"/>
            <Switch v-model="attribute.have_child" :errors="v$.attribute.have_child.$errors" class="col-span-12 sm:col-span-4" :label="$t('forms.attributes.have_child')" name="active-input"/>
      ...
      >
        ...
        <template #table-row="props">
          ...
          <span v-if="props.column.field == 'actions'">
              ...
              <button v-if="!props.row.deleted" class="flex items-center btn btn-secondary  w-24 mr-1 mb-2" v-can="['edit-attribute']"  @click="OpenNewAttributeModal(props.row.id)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" name="CheckSquare"/>{{ $t('g.edit') }}
              </button>
              <router-link v-if="!props.row.deleted && !hasChildren && props.row.have_child" class="flex items-center btn btn-secondary  w-24 mr-1 mb-2" v-can="['edit-product']" :to="{name:'sub_attributes' ,params : {attribute:props.row.id} }" >
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" name="CheckSquare"/>{{ $t('g.show') }}
              </router-link>

              <button v-if="props.row.deleted" class="flex items-center btn btn-success text-white  w-24 mr-1 mb-2" v-can="['restore-attribute']" @click="restoreAttribute(props.row.id , props.row.name)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" color="white" name="ArrowLeft"/>{{ $t('g.restore') }}
              </button>
              <button class="flex items-center btn btn-danger w-24 mr-1 mb-2"  v-can="['delete-attribute']" @click="deleteAttribute(props.row.id , props.row.name)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
          ...
          <span v-if="props.column.field == 'have_child'">
            <Switch v-model="props.row.have_child" :disabled="$h.checkBoolean(serverParams.deleted)" name="have_child" @change="haveChildChange(props.row.id)"/>
          </span>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [ // Columns For Table
        ...
        {
          label: this.$t('forms.attributes.have_child'),
          field: "have_child",
          hasTranslate:true,
          tdClass: "text-left",
          thClass: "text-left",
  ...
  methods: {
    ...
    async getData(page = null) {
      ...
      let data = await AttributeController.getData(this.serverParams);
    ...
    async getAttributes() {
      ...
      let data = await AttributeController.getData({length: 'all',parent_id:0});

      if(data){


        let parent =  {id:0 , name : {}};
        await window.locales.forEach(local => {

          var lock = {
            [local]:  'Select as Parant',
          }
        ...
        this.attributes.push(parent)

        let newArray = this.attributes.concat(data.data);
    ...
    async getParent(id) {
      this.StartLoading();
      let data = await AttributeController.getModel(id);
      if(data){
          this.parent = data;
      }
      this.StopLoading();
    ...
    OpenViewAttributeModal(id = null) {
      this.resetForm();
    ...
    async statusChange(id) {
      this.StartLoading();
      let response = await AttributeController.ToggleActive(id);
   ...
   //---- Change Status Handler
   async haveChildChange(id) {
      this.StartLoading();
      let response = await AttributeController.ToggleHaveChild(id);
      if(response && response.status == 'success'){
        $h.notify(this.$t('messages.success'), response.message);
      }
      this.getData();
    ...
    async save(e , addNew = false) {
      ...
      this.attribute.parent_id = this.parent_id;
      if (this.editMode) {
        let response = await AttributeController.update(this.attribute);
        if(response && response.status == 'success'){
          this.getData();
        ...
        let response = await AttributeController.store(this.attribute);
        if(response && response.status == 'success'){
          this.getData();
    ...
    async deleteAttribute(id , name = "") {
      ...
      let response = await AttributeController.delete(id);
      if(response && response.status == 'success'){
        this.getData();
        $h.notify(this.$t('messages.success'), response.message);
      }
    ...
    async restoreAttribute(id , name = "") {
      this.StartLoading();
      const answer = window.confirm(this.$t('messages.confirmations.restore' , {item: name}))
      if (!answer){ this.StopLoading();  return false}

      let response = await AttributeController.restore(id);
      if(response && response.status == 'success'){
        this.getData();
        $h.notify(this.$t('messages.success'), response.message);
      }
  ...
  created() {
    ...
    this.handelParentsAttr(this.$route.params.attribute);
