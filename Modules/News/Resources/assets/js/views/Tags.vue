<template>
  ...
  <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">{{ !$h.checkBoolean(serverParams.deleted) ?   $t('g.list', {model: $t('tags.plural')})  : $t('g.deleted', {model: $t('tags.plural')}) }}</h2>
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
      <button class="btn btn-primary shadow-md mr-2" v-can="['create-tag']" @click="OpenNewTagModal()">
        <PlusIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.create') }}
      </button>
      <a v-can="['restore-tag','force-delete-tag']" v-if="$h.checkBoolean(serverParams.deleted)" class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,false)">
        <ListIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.list', {model: $t('tags.plural')}) }}
      </a>
      <a v-can="['restore-tag','force-delete-tag']" v-if="!$h.checkBoolean(serverParams.deleted)" class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,true)">
        <Trash2Icon class="mr-2 w-5 h-5"/>
        {{ $t('g.deleted', {model: $t('tags.plural')}) }}
      </a>
      <!-- BEGIN: Modal Content -->
      <Modal v-can="['create-tag','edit-tag']" :show="ModelIsOpen" size="modal-xl" @hidden="closeModel()">
        <form class="validate-form" @submit.prevent="save">
          <ModalHeader>
            <h2 class="font-medium text-base mr-auto">
              {{
                editMode ? $t('g.edit_row', {model: $t('tags.plural')}) : $t('g.add_new', {model: $t('tags.plural')})
              }}
            </h2>
          </ModalHeader>
          <ModalBody class="grid grid-cols-12 gap-8 gap-y-4">
            ...
            <TabGroup class="col-span-12 sm:col-span-12">
              ...
              <TabPanels class="border-l border-r border-b">
                <template v-for="(local, index) in locales" :key="index">
                  ...
                  <TabPanel class="p-5">
                    <InputField v-model="tag.name[local]" :errors="v$.tag.name[local].$errors" class="col-span-12 sm:col-span-4"
            ...
            <Switch v-model="tag.active" :errors="v$.tag.active.$errors" class="col-span-12 sm:col-span-4" :label="$t('forms.attributes.active')" name="active-input"/>


            <!-- <SelectField
              v-model="tag.role"
              :errors="v$.tag.role.$errors"
              labelValue="name"
              keyValue="id"
              :selectData="roles"
      ...
      >
        ...
        <template #table-row="props">
          ...
          <span v-if="props.column.field == 'actions'">
              ...
              <button v-if="!props.row.deleted" class="flex items-center btn btn-secondary  w-24 mr-1 mb-2" v-can="['edit-tag']"  @click="OpenNewTagModal(props.row.id)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" name="CheckSquare"/>{{ $t('g.edit') }}
              </button>

              <button v-if="props.row.deleted" class="flex items-center btn btn-success text-white  w-24 mr-1 mb-2" v-can="['restore-tag']" @click="restoreTag(props.row.id , props.row.name)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" color="white" name="ArrowLeft"/>{{ $t('g.restore') }}
              </button>
              <button class="flex items-center btn btn-danger w-24 mr-1 mb-2"  v-can="['delete-tag']" @click="deleteTag(props.row.id , props.row.name)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
...
export default {
  ...
  methods: {
    ...
    async getData(page = null) {
      ...
      let data = await TagController.getData(this.serverParams);
    ...
    async getModel(id) {
      this.StartLoading();
      let data = await TagController.getModel(id);
      if(data){
          this.tag = data;
      }
      this.StopLoading();
    ...
    async statusChange(id) {
      this.StartLoading();
      let response = await TagController.ToggleActive(id);
    ...
    async save(e , addNew = false) {
      ...
      if (this.editMode) {
        let response = await TagController.update(this.tag);
        if(response && response.status == 'success'){
          this.getData();
          $h.notify(this.$t('messages.success'), response.message);
        ...
        let response = await TagController.store(this.tag);
        if(response && response.status == 'success'){
          this.getData();
          $h.notify(this.$t('messages.success'), response.message);
          this.handelFinishRequest(addNew);
    ...
    async deleteTag(id , name = "") {
      ...
      let response = await TagController.delete(id);
      if(response && response.status == 'success'){
        this.getData();
        $h.notify(this.$t('messages.success'), response.message);
      }
