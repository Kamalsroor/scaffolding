<template>
  ...
  <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">{{ !$h.checkBoolean(serverParams.deleted) ?   $t('g.list', {model: $t('subscription.plural')})  : $t('g.deleted', {model: $t('subscription.plural')}) }}</h2>
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
      <!-- <button class="btn btn-primary shadow-md mr-2" v-can="['create-subscribe']" @click="OpenNewSubscribeModal()">
        <PlusIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.create') }}
      </button> -->
      <a v-can="['restore-subscribe','force-delete-subscribe']" v-if="$h.checkBoolean(serverParams.deleted)" class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,false)">
        <ListIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.list', {model: $t('subscription.plural')}) }}
      </a>
      <a v-can="['restore-subscribe','force-delete-subscribe']" v-if="!$h.checkBoolean(serverParams.deleted)" class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,true)">
        <Trash2Icon class="mr-2 w-5 h-5"/>
        {{ $t('g.deleted', {model: $t('subscription.plural')}) }}
      </a>
      <!-- BEGIN: Modal Content -->
      <Modal v-can="['create-subscribe','edit-subscribe']" :show="ModelIsOpen" size="modal-xl" @hidden="closeModel()">
        <form class="validate-form" @submit.prevent="save">
          <ModalHeader>
            <h2 class="font-medium text-base mr-auto">
              {{
                editMode ? $t('g.edit_row', {model: $t('subscription.plural')}) : $t('g.add_new', {model: $t('subscription.plural')})
              }}
            </h2>
          </ModalHeader>
          <ModalBody class="grid grid-cols-12 gap-8 gap-y-4">

              <!-- <InputField type="number" v-model="subscribe.order_id" :errors="v$.subscribe.order_id.$errors" class="col-span-12 sm:col-span-4"
                          :label="$t('forms.attributes.order_id')" name="order_id" :placeholder="$t('forms.attributes.order_id')"/>
 -->

            <!-- <Switch v-model="subscribe.active" :errors="v$.subscribe.name.$errors" class="col-span-12 sm:col-span-4" :label="$t('forms.attributes.active')" name="active-input"/> -->
            <!-- <SelectField
              v-model="subscribe.role"
              :errors="v$.subscribe.role.$errors"
              labelValue="name"
              keyValue="id"
              :selectData="roles"
        ...
        <template #table-row="props">
          ...
          <span v-if="props.column.hasTranslate">
            ...
            <template v-for="(item, index) in props.row[props.column.field]" :key="index">
              <div class="font-medium text-gray-900 truncate"> {{ index }} : {{ item[props.column.field] }} ,</div>
            </template>
          </span>
...
<!--
              <button v-if="!props.row.deleted" class="flex items-center btn btn-secondary  w-24 mr-1 mb-2" v-can="['edit-subscribe']"  @click="OpenNewSubscribeModal(props.row.id)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" name="CheckSquare"/>{{ $t('g.edit') }}
              </button> -->

              <button v-if="props.row.deleted" class="flex items-center btn btn-success text-white  w-24 mr-1 mb-2" v-can="['restore-subscribe']" @click="restoreSubscribe(props.row.id , props.row.name)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" color="white" name="ArrowLeft"/>{{ $t('g.restore') }}
              </button>
              <button class="flex items-center btn btn-danger w-24 mr-1 mb-2"  v-can="['delete-subscribe']" @click="deleteSubscribe(props.row.id , props.row.name)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
...
import SubscribeController from '@@/CommonData/Resources/assets/js/controllers/SubscribeController.js';
...
export default {
  ...
  methods: {
    ...
    async getData(page = null) {
      ...
      let data = await SubscribeController.getData(this.serverParams);
    ...
    async getModel(id) {
      this.StartLoading();
      let data = await SubscribeController.getModel(id);
      if(data){
          this.subscribe = data;
      }
      this.StopLoading();
    ...
    async statusChange(id) {
      this.StartLoading();
      let response = await SubscribeController.ToggleActive(id);
    ...
    async save(e , addNew = false) {
      ...
      if (this.editMode) {
        let response = await SubscribeController.update(this.subscribe);
        if(response && response.status == 'success'){
          this.getData();
          $h.notify(this.$t('messages.success'), response.message);
        ...
        let response = await SubscribeController.store(this.subscribe);
        if(response && response.status == 'success'){
          this.getData();
          $h.notify(this.$t('messages.success'), response.message);
          this.handelFinishRequest(addNew);
    ...
    async restoreSubscribe(id , name = "") {
      this.StartLoading();
      const answer = window.confirm(this.$t('messages.confirmations.restore' , {item: name}))
      if (!answer){ this.StopLoading();  return false}

      let response = await SubscribeController.restore(id);
      if(response && response.status == 'success'){
        this.getData();
        $h.notify(this.$t('messages.success'), response.message);
      }
