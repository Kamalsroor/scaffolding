<template>
  ...
  <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">{{ !$h.checkBoolean(serverParams.deleted) ?   $t('g.list', {model: $t('contact_us.plural')})  : $t('g.deleted', {model: $t('contact_us.plural')}) }}</h2>
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
      <!-- <button class="btn btn-primary shadow-md mr-2" v-can="['create-contact_us']" @click="OpenNewContactUsModal()">
        <PlusIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.create') }}
      </button> -->
      <a v-can="['restore-contact_us','force-delete-contact_us']" v-if="$h.checkBoolean(serverParams.deleted)" class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,false)">
        <ListIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.list', {model: $t('contact_us.plural')}) }}
      </a>
      <a v-can="['restore-contact_us','force-delete-contact_us']" v-if="!$h.checkBoolean(serverParams.deleted)" class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,true)">
        <Trash2Icon class="mr-2 w-5 h-5"/>
        {{ $t('g.deleted', {model: $t('contact_us.plural')}) }}
      </a>
      ...
      <!-- BEGIN: Modal Content -->
      <Modal v-can="['create-contact_us','edit-contact_us']" :show="ModelIsOpen" size="modal-xl" @hidden="closeModel()">
          <ModalHeader>
          ...
          <div class="mb-6 ">
            ...
            <span>{{contact_us.description}}</span>
          </div>
            ...
            <form id="tabulator-html-filter-form" @submit="onFilter"  class="sm:mr-auto">
                <div class=" flex flex-wrap gap-3 p-3  bg-slate-100 border border-dashed border-slate-200 rounded-md">
                    ...
                    <div class="flex items-center sm:mr-4 grow">
                        ...
                        <select id="tabulator-html-filter-field" v-model="serverParams.columnFilters.field"
                            ...
                            <option value="phone">{{ $t('forms.attributes.phone') }}</option>
                        </select>
                    </div>
                ...
                <div class=" flex flex-wrap gap-3 p-3 mt-4   bg-slate-100 border border-dashed border-slate-200 rounded-md">
                    ...
                    <InputField v-model="serverParams.search.email"  class="col-span-12 sm:col-span-12"
                        ...
                        <InputField v-model="serverParams.search.phone"  class="col-span-12 sm:col-span-12"
                        :label="$t('forms.attributes.phone')" name="phone"  :placeholder="$t('forms.attributes.phone')"/>
                </div>
        ...
        <template #table-row="props">
          ...
          <span v-if="props.column.field == 'actions'">
              <div class="flex lg:justify-center items-center">
                  ...
                  <!-- Start View Button  -->
              <button v-if="!props.row.deleted" class="flex items-center btn btn-secondary  w-24 mr-1 mb-2" v-can="['edit-contact_us']"  @click="OpenNewContactUsModal(props.row.id , props.row.show_date)">
                <!-- End View Button  -->
              ...
              <button v-if="props.row.deleted" class="flex items-center btn btn-success text-white  w-24 mr-1 mb-2" v-can="['restore-contact_us']" @click="restoreContactUs(props.row.id , props.row.name)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" color="white" name="ArrowLeft"/>{{ $t('g.restore') }}
              </button>
              <button class="flex items-center btn btn-danger w-24 mr-1 mb-2"  v-can="['delete-contact_us']" @click="deleteContactUs(props.row.id , props.row.name)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
...
import ContactUsController from '@@/CommonData/Resources/assets/js/controllers/ContactUsController.js';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [ // Columns For Table
        ...
        {
          label: this.$t('forms.attributes.phone'),
          field: "phone",
          tdClass: "text-left",
          thClass: "text-left",
          sortable: true,
        ...
        {
          label: this.$t('forms.attributes.description'),
          field: "description",
          tdClass: "text-left",
          thClass: "text-left",
          sortable: true,
  ...
  methods: {
    ...
    async getData(page = null) {
      ...
      let data = await ContactUsController.getData(this.serverParams);
    ...
    async getModel(id) {
      this.StartLoading();
      let data = await ContactUsController.getModel(id);
      if(data){
    ...
    OpenNewContactUsModal(id = null , show_date = null) {
      this.resetForm();
      this.openModel();
    ...
    async statusChange(id) {
      this.StartLoading();
      let response = await ContactUsController.ToggleActive(id);
    ...
    rowStyleClassFn(row) {
      return row.show_date ? '' : 'vgt-row-gray-50';
    },
    ...
    async save(e , addNew = false) {
      ...
      if (this.editMode) {
        let response = await ContactUsController.update(this.contact_us);
        if(response && response.status == 'success'){
          this.getData();
          $h.notify(this.$t('messages.success'), response.message);
        ...
        let response = await ContactUsController.store(this.contact_us);
        if(response && response.status == 'success'){
          this.getData();
          $h.notify(this.$t('messages.success'), response.message);
          this.handelFinishRequest(addNew);
    ...
    async restoreContactUs(id , name = "") {
      this.StartLoading();
      const answer = window.confirm(this.$t('messages.confirmations.restore' , {item: name}))
      if (!answer){ this.StopLoading();  return false}

      let response = await ContactUsController.restore(id);
      if(response && response.status == 'success'){
        this.getData();
        $h.notify(this.$t('messages.success'), response.message);
      }
