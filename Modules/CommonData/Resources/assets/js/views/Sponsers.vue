<template>
  ...
  <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    ...
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
      ...
      <a v-can="['restore-sponser','force-delete-sponser']" v-if="$h.checkBoolean(serverParams.deleted)" class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,false)">
        <ListIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.list', {model: $t('sponsers.plural')}) }}
      </a>
      <a v-can="['restore-sponser','force-delete-sponser']" v-if="!$h.checkBoolean(serverParams.deleted)" class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,true)">
        <Trash2Icon class="mr-2 w-5 h-5"/>
        {{ $t('g.deleted', {model: $t('sponsers.plural')}) }}
      </a>
      <!-- BEGIN: Modal Content -->
      <Modal v-can="['create-sponser','edit-sponser']" :show="ModelIsOpen" size="modal-xl" @hidden="closeModel()">
        <form class="validate-form" @submit.prevent="save">
          <ModalHeader>
            <h2 class="font-medium text-base mr-auto">
              {{
                editMode ? $t('g.edit_row', {model: $t('sponsers.plural')}) : $t('g.add_new', {model: $t('sponsers.plural')})
              }}
            </h2>
          </ModalHeader>
      ...
      >
        ...
        <template #table-row="props">
          <span v-if="props.column.field == 'actions'">
              ...
              <button v-if="!props.row.deleted" class="flex items-center btn btn-secondary  w-24 mr-1 mb-2" v-can="['edit-sponser']"  @click="OpenNewSponserModal(props.row.id)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" name="CheckSquare"/>{{ $t('g.edit') }}
              </button>

              <button v-if="props.row.deleted" class="flex items-center btn btn-success text-white  w-24 mr-1 mb-2" v-can="['restore-sponser']" @click="restoreSponser(props.row.id , props.row.name)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" color="white" name="ArrowLeft"/>{{ $t('g.restore') }}
              </button>
              <button class="flex items-center btn btn-danger w-24 mr-1 mb-2"  v-can="['delete-sponser']" @click="deleteSponser(props.row.id , props.row.name)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
...
export default {
  ...
  methods: {
    ...
    async getData(page = null) {
      ...
      let data = await SponserController.getData(this.serverParams);
    ...
    async getModel(id) {
      this.StartLoading();
      let data = await SponserController.getModel(id);
      if(data){
          this.sponser = data;
      }
      this.StopLoading();
    ...
    async save(e , addNew = false) {
      ...
      if (this.editMode) {
        let response = await SponserController.update(this.sponser);
        if(response && response.status == 'success'){
          this.getData();
          $h.notify(this.$t('messages.success'), response.message);
        ...
        let response = await SponserController.store(this.sponser);
        if(response && response.status == 'success'){
          this.getData();
          $h.notify(this.$t('messages.success'), response.message);
          this.handelFinishRequest(addNew);
    ...
    async restoreSponser(id , name = "") {
      this.StartLoading();
      const answer = window.confirm(this.$t('messages.confirmations.restore' , {item: name}))
      if (!answer){ this.StopLoading();  return false}

      let response = await SponserController.restore(id);
      if(response && response.status == 'success'){
        this.getData();
        $h.notify(this.$t('messages.success'), response.message);
      }
    ...
    onExport(type){
      this.axios.get(`export/${type}`, {
        params: {
          model : 'Sponser',
          export : 'Export',
          repo : 'SponserRepository',
          resource : 'SponserResource',
          file_name : 'Sponsers',
          ...this.serverParams},
