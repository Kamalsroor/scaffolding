<template>
  ...
  <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">{{ !$h.checkBoolean(serverParams.deleted) ?   $t('g.list', {model: $t('roles.plural')})  : $t('g.deleted', {model: $t('roles.plural')}) }}</h2>
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
      <a class="btn btn-primary shadow-md mr-2" v-can="['create-role']" @click="OpenNewRoleModal()">
        <PlusIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.create') }}
      </a>
      <a v-can="['restore-role','force-delete-role']" v-if="$h.checkBoolean(serverParams.deleted)" class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,false)">
        <ListIcon class="mr-2 w-5 h-5"/>
        {{ $t('g.list', {model: $t('roles.plural')}) }}
      </a>
      <a v-can="['restore-role','force-delete-role']" v-if="!$h.checkBoolean(serverParams.deleted)" class="btn btn-primary shadow-md mr-2" @click="ToggleDeletedData(true,true)">
        <Trash2Icon class="mr-2 w-5 h-5"/>
        {{ $t('g.deleted', {model: $t('roles.plural')}) }}
      </a>
      <!-- BEGIN: Modal Content -->
      <Modal v-can="['create-role','edit-role']" :show="ModelIsOpen" size="modal-xl" @hidden="closeModel()">
        <form class="validate-form" @submit.prevent="save">
          <ModalHeader>
            <h2 class="font-medium text-base mr-auto">
              {{
                editMode ? $t('g.edit_row', {model: $t('roles.plural')}) : $t('g.add_new', {model: $t('roles.plural')})
              }}
            </h2>
          </ModalHeader>
          <ModalBody class="grid grid-cols-12 gap-8 gap-y-4">
            <InputField v-model="role.name" :errors="v$.role.name.$errors" class="col-span-12 sm:col-span-4"
                        :label="$t('forms.attributes.name')" name="name" :placeholder="$t('forms.attributes.name')"/>


            <Switch v-model="role.active" :errors="v$.role.name.$errors" class="col-span-12 sm:col-span-4" :label="$t('forms.attributes.active')" name="active-input"/>


            <section class="col-span-12 grid grid-cols-12 gap-8 gap-y-4">
                <div class="col-span-12 sm:col-span-4 border border-dashed border-slate-400" v-for="(group, index) in permissions" :key="index">
                      <Switch v-model="role.permissions" labelValue="title"
                            keyValue="name"
                            :name="index"
                            :multiple="true"
                            :selectData="group" :errors="v$.role.name.$errors" class="col-span-12 sm:col-span-12" :label="index" name="active-input"/>
      ...
      >
        ...
        <template #table-row="props">
          <span v-if="props.column.field == 'actions'">
              ...
              <button v-if="!props.row.deleted" class="flex items-center btn btn-secondary  w-24 mr-1 mb-2" v-can="['edit-role']"  @click="OpenNewRoleModal(props.row.id)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" name="CheckSquare"/>{{ $t('g.edit') }}
              </button>

              <button v-if="props.row.deleted" class="flex items-center btn btn-success text-white  w-24 mr-1 mb-2" v-can="['restore-role']" @click="restoreRole(props.row.id , props.row.name)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
                    <Icon :show="!isLoading" class="w-4 h-4 mr-1" color="white" name="ArrowLeft"/>{{ $t('g.restore') }}
              </button>
              <button class="flex items-center btn btn-danger w-24 mr-1 mb-2"  v-can="['delete-role']" @click="deleteRole(props.row.id , props.row.name)">
                    <LoadingIcon :show="isLoading" icon="three-dots" color="white" class="w-4 h-4 mr-2" />
...
import PermissionController from '@@/Admin/Resources/assets/js/controllers/PermissionController.js';
...
export default {
  ...
  methods: {
    ...
    async getData(page = null) {
      ...
      let data = await RoleController.getData(this.serverParams);
    ...
    async getPermissions() {
      this.StartLoading();
      let data = await PermissionController.getData();
      if(data){
    ...
    async getModel(id) {
      this.StartLoading();
      let data = await RoleController.getModel(id);
      if(data){
          this.role = data;
      }
    ...
    async statusChange(id) {
      this.StartLoading();
      let response = await RoleController.ToggleActive(id);
    ...
    async save(e , addNew = false) {
      ...
      if (this.editMode) {
        let response = await RoleController.update(this.role);
        if(response && response.status == 'success'){
          this.getData();
          $h.notify(this.$t('messages.success'), response.message);
        ...
        let response = await RoleController.store(this.role);
        if(response && response.status == 'success'){
          this.getData();
          $h.notify(this.$t('messages.success'), response.message);
          this.handelFinishRequest(addNew);
    ...
    async deleteRole(id , name = "") {
      ...
      let response = await RoleController.delete(id);
      if(response && response.status == 'success'){
        this.getData();
        $h.notify(this.$t('messages.success'), response.message);
      }
