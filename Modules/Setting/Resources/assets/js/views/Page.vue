          ...
          <div class="col-span-12 ">
            <div class="box intro-y my-8 first:my-0 p-5">
              <div class="flex items-center border-b border-slate-200/60 dark:border-darkmode-400 pb-5 mb-5">
                <div class="font-medium text-base truncate">{{ settings?.title }}</div>
              </div>
              <div>
                ...
                <TabGroup :horizontal="true" v-if="settings" :selectedIndex="selectedIndex">
                    <TabList :horizontal="true" class="nav-link-tabs">
                      <template  v-for="(group , index) in settings.section" :key="index">
                        ...
                        <TabPanel  v-if="group.elements"  class="leading-relaxed">
                            ...
                            :is-full-page="false" loader="dots" transition="bounce"></LoadingVue>
                              <template v-for="element in group.elements" :key="element.name">
                                ...
                                <div v-if="tab === index && !isLoading" class="my-8">
...
export default {
  ...
  methods: {
    ...
    async getData(page = null) {
      this.StartLoading();
      let data = await SettingController.getDataByPage(this.page);
      this.inputs = {};
      if(data){
          this.settings = data.data.data;
          // if(this.tab === null){
            this.tab = data.data.tab;
          // }
    ...
    async handelInputs(){
      ...
      await Object.entries(this.settings.section).forEach(([key, field]) => {
        if (key == this.tab) {
          if(field.elements){
    ...
    changePage(tab){
      this.$router.replace({name: this.$route.name, query: {tab: tab} , params : {page : this.page}})
      this.tab =  tab;
    },
  ...
  computed:{
    selectedIndex() {
      ...
      let elementIndex = -1;
      Object.entries(this.settings.section).every((key, field) => {
        elementIndex++;
        if (key[0] ==  this.tab) {
          return false
        }
  ...
  created() {
    ...
    if(this.$route.params){
      this.page = this.$route.params.page ?? 'home';
    }
    if(this.$route.query){
      this.tab = this.$route.query.tab ?? 'header';
    }
